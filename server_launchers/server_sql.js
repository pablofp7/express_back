...
const { userDbType, movieDbType, tokenTransport } = yargs(hideBin(process.argv))
  .option('userDbType', {
    alias: 'u',
  ...
  .option('tokenTransport', {
    alias: 't',
    describe: 'How to send the token: cookie or header',
...
;(async () => {
  try {
    ...
    createApp({ movieModel, userModel, tokenTransport })
  }
  catch (error) {
